...
def test_non_uniform_data_types():
    with pytest.raises(
        ValueError, match="All features must be of the same type across selections."
    ):
...
def test_identical_feature_selections_strings():
    ...
    assert (
        stability == 1
    ), "Stability should be 1 when all feature selections are identical (strings)"
...
def test_edge_case_no_features():
    with pytest.raises(
        ValueError, match="Feature selections cannot contain empty sets."
    ):
