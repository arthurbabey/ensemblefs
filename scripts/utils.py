...
def validate_pipeline_config(config):
    # Validate `fs_methods`
    fs_methods = config.get("fs_methods", {}).get("value", [])
    valid_fs_methods = {
    ...
    merging_strategy = config.get("merging_strategy", {}).get("value", "")
    ...
    # validate metrics
    metrics = config.get("metrics", {}).get("value", [])
    valid_metrics = {
        "accuracy",
        "logloss",
        "f1_score",
    ...
    if not isinstance(metrics, list) or not all(
        ...
        raise ValueError(
            f"`metrics` should be a list containing exactly three metrics names from the following options: {valid_metrics}."
        )

    # Validate `task`
    task = config.get("task", {}).get("value", "")
...
def validate_experience_config(config):
    # Validate `data_path`
